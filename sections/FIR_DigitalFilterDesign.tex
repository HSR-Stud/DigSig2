...
\begin{tabular}{|l|c|c|c|c|c|c|}
	...
	Kaiser & variable $\delta$ & $-20log_{10}\delta$ & $17.372\delta$ & $(A-7.95)/14.36$ & variable R & $6(R+12)/55$\\
	\hline
\end{tabular}
...
If the analog signal includes sinusoid with greatly varying amplitudes, some of them might get lost in the sidelobes of the window. The Kaiser Window allows for a variable sidelobe suppression (\ref{window_comparison}).\newline
Instead of $A$, $R$ is used to calculate $\alpha$:
\begin{align*}
	\alpha = \left\{
		\begin{array}{l l}
			0 & \quad \text{if } R < 13.26\\
			0.76609(R-13.26)^{0.4}+0.09834(R-13.26) & \quad \text{if }13.26 < R < 60 \\
			0.12438(R+6.3) & \quad \text{if }60 < R < 120 \\
		\end{array} \right. \\
\end{align*}
Calculate $c=\frac{6(R+12)}{155}$ and $L=1+\frac{cf_s}{\Delta f}$ and round L to the next \textbf{odd} number and continue with step 6 (\ref{design_steps}).
